...
the input is signalled by a negative loan duration
...
Output
For each loan, the output is the number of complete months before the borrower 
owes less than the car is worth. Note that English requires plurals (5 months) 
on all values other than one (1 month).
...
def loan(num_months, down, principal, records):
    amount = down + principal

    month, percent = 0, 0

    for rec_month, rec_percent in records:
        while amount 
        amount *= (1 - percent) ** (rec_month - month) 
        month, percent = rec_month, rec_percent

    amount *= percent ** (num_months - month)
