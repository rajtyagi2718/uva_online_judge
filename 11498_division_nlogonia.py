...
(0 < K ≤ 1e3). The second line of a test case contains two integers N and M 
representing the coordinates of the division point (−1e4 < N, M < 1e4). Each
of the K following lines contains two integers X and Y representing the 
...
def divisa_factory(m, n):
    """Return divisa function for division lines at n and m."""

    divstr = 'divisa'
    
    def divisa(x, y):
        """Return string for coordinate direction relative to division line."""
        if y < n:
            result = 'N'
        elif y > n:
            result = 'S'
