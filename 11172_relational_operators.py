...
is just to find out the relationship between them
that is (i) First one is greater than the second (ii) First one is less than 
...
‘>’, ‘<’ or ‘=’, which indicates the relation that is appropriate for the given
two numbers.
...
def relop(n, m):
    """Return string < > = for correct comparison between integers."""
    if n < m:
        return '<'
    elif n > m:
        return '>'
